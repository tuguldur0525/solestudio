      ...
      <Link to="/products/$slug" params={{ slug: product.slug }} className="block">
        ...
        <div className="relative overflow-hidden bg-bone p-2">
          <div className="relative overflow-hidden bg-cement/20">
            ...
            <div className="absolute left-2 top-3 flex flex-col items-start gap-1">
              {product.new_arrival && (
                <span className="bg-ink px-2 py-1 font-mono text-[10px] font-bold tracking-[0.12em] text-ink-foreground">NEW</span>
              )}
              {onSale && (
                <span className="bg-primary px-2 py-1 font-mono text-[10px] font-bold tracking-[0.12em] text-primary-foreground">SALE</span>
              )}
            </div>
        ...
        <div className="flex flex-col gap-2 px-3 pb-4 pt-3">
          <div className="flex items-start justify-between gap-3">
            <h3 className="font-sans text-[13px] font-semibold leading-tight tracking-wide">{product.name.toUpperCase()}</h3>
            <span className="shrink-0 font-mono text-[11px] text-muted-foreground">{product.product_variants?.length ?? 0} SZ</span>
          </div>

          <div className="flex items-baseline gap-2">
            {onSale && <span className="font-mono text-xs text-muted-foreground line-through">{formatMnt(product.price)}</span>}
            <span className={`font-mono text-sm font-semibold ${onSale ? "text-primary" : "text-foreground"}`}>
          ...
          {colors.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <p className="font-mono text-[11px] tracking-wide text-muted-foreground">{colors.join(" · ").toUpperCase()}</p>
            </div>
          )}
        </div>
      </Link>
